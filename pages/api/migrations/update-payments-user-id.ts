...
import { updatePaymentsUserIdType } from '../../../lib/migrations/update-payments-user-id';
...
export default async function handler(
  ...
  try {
    // Run the migration
    const result = await updatePaymentsUserIdType();
    
    // Return success
    return res.status(200).json({
      success: true,
      message: 'Payments table user_id updated to work with NextAuth',
      result
    });
  } catch (error: any) {
